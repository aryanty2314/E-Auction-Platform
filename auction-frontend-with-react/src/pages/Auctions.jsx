...
function Auctions() {
  ...
  return (
    <div className="min-h-screen bg-black text-white px-8 py-10">
      <h1 className="text-3xl font-bold mb-6 text-green-400 text-center">🔥 Live Auctions</h1>

      {auctions.length === 0 ? (
        <p className="text-gray-400 text-center">No active auctions right now.</p>
      ) : (
        <div className="grid gap-6 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
          {auctions.map((auction) => (
            <div
              key={auction.id}
              className="bg-gray-900 border border-gray-700 p-5 rounded-xl shadow-lg"
            >
              <img
